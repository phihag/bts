...
function reset_player_tabletoperator(app, tournament_key, match_id, end_ts) {
	const match_utils = require('./match_utils'); // avoid dependency cycle
	async.waterfall([
		cb => match_utils.remove_player_on_court(app, tournament_key, match_id, end_ts, cb),
		cb => match_utils.remove_tablet_on_court(app, tournament_key, match_id, end_ts, cb),
		cb => match_utils.remove_umpire_on_court(app, tournament_key, match_id, end_ts,cb),
		cb => match_utils.add_player_to_tabletoperator_list(app, tournament_key, match_id, end_ts, cb)
