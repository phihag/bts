	...
	async.each(btp_state.matches, function(bm, cb) {
		...
		app.db.matches.findOne(query, (err, cur_match) => {
			...
			if (cur_match) {
				...
				if (cur_match.setup.tabletoperators_btp_ids) {
					// tabletoperators_btp_ids is not from btp so we have to coppy it to the match generated by btp.
					match.setup.tabletoperators_btp_ids = cur_match.setup.tabletoperators_btp_ids;
				}
...
async function integrate_now_on_court(app, tkey, callback) {
	...
	app.db.tournaments.findOne({key: tkey}, async (err, tournament) => {
		...
		app.db.matches.find({'setup.now_on_court': true}, async (err, now_on_court_matches) => {
			...
			async.each(now_on_court_matches, async (match, cb) => {
				...
				if(!setup.called_timestamp) {
					...
					try{
						const [tabletoperators, tabletoperators_btp_ids] = await get_last_looser_on_court(app, tkey, court_id, setup.umpire_name);
						setup.tabletoperators = tabletoperators;
						setup.tabletoperators_btp_ids = tabletoperators_btp_ids;
					} catch (err) {
						callback(err)
					}
...
function get_last_looser_on_court(app, tkey, court_id, umpire_name) {
	return new Promise((resolve, reject) => {
		...
		let tabletoperators_btp_ids = undefined;
		...
		app.db.matches.find(match_querry).sort({'end_ts': -1}).limit(1).exec((err, last_match_on_court) => {
			...
			if (last_match_on_court.length === 1 && !umpire_name) {
				...
				if(team && typeof team.players !== 'undefined') {
					...
					tabletoperators_btp_ids = [];
					if (last_match_on_court[0].btp_player_ids.length == 2) {
						tabletoperators_btp_ids.push(
							(last_match_on_court[0].btp_winner == 1 ? last_match_on_court[0].btp_player_ids[1] : last_match_on_court[0].btp_player_ids[0]));
					} else if (last_match_on_court[0].btp_player_ids.length == 4){
						tabletoperators_btp_ids.push(
							(last_match_on_court[0].btp_winner == 1 ? last_match_on_court[0].btp_player_ids[2] : last_match_on_court[0].btp_player_ids[0]));
						tabletoperators_btp_ids.push(
							(last_match_on_court[0].btp_winner == 1 ? last_match_on_court[0].btp_player_ids[3] : last_match_on_court[0].btp_player_ids[1]));
					}
				}
			}

			resolve([tabletoperators, tabletoperators_btp_ids]);
		});
	});
}
